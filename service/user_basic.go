...
func SendCode(c *gin.Context) {
	...
	err = models.RDB.Set(context.Background(), define.RegisterPerfix+email, code, time.Second*time.Duration(define.ExpireTime)).Err()
	if err != nil {
		log.Println("[ERROR]:", err)
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
...
func Register(c *gin.Context) {
	...
	// 判断账号是否唯一
	count, err := models.GetUserBasicCountByAccount(account)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
			"msg":  "系统错误",
	...
	r, err := models.RDB.Get(context.Background(), define.RegisterPerfix+email).Result()
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
			"msg":  "验证码不正确",
